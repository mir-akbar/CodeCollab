/**
 * Unified File Storage Service with Y-WebSocket Integration
 * Complete file management solution combining MongoDB persistence with real-time Y-WebSocket updates
 * Replaces the old fileStorageService with enhanced ZIP processing and error handling
 */

class FileUploadYjsService {
  constructor(yjsServer) {
    this.yjsServer = yjsServer;
    this.uploadSessions = new Map(); // Track active uploads
  }

  /**
   * Upload single file with real-time progress via Y-WebSocket
   */
  async uploadFile(sessionId, fileData, uploaderEmail) {
    const uploadId = `upload-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    
    try {
      console.log(`📤 Starting file upload: ${fileData.fileName} for session ${sessionId}`);
      
      // Store file in MongoDB using our own storeFile method
      ...
      console.log(`✅ File stored in MongoDB: ${fileData.fileName}`);
      ...
      console.error(`❌ File upload failed: ${fileData.fileName}`, error);
      throw error;
    }
  }

  /**
   * Upload ZIP file with real-time progress
   */
  async uploadZipFile(sessionId, zipFile, uploaderEmail) {
    const uploadId = `zip-upload-${Date.now()}`;
    
    try {
      console.log(`📦 Starting ZIP upload for session ${sessionId}`);
      ...
      console.log(`✅ ZIP processing complete: ${extractedFiles.length} files extracted`);
    ...
    return new Promise((resolve, reject) => {
      ...
      entry.on('end', async () => {
        try {
          ...
          const parentFolder = require('path').dirname(fileName) !== '.' ? require('path').dirname(fileName) : null;
  ...
  /**
   * Get session files (proxy to existing service)
   */
  ...
  /**
   * Store file with enhanced logic (replaces fileStorageService.storeFile)
   */
  async storeFile(fileData) {
    try {
      ...
      await file.save();
      console.log(`✅ File stored: ${fileData.fileName} (${file.fileSize} bytes)`);
      return file;
    } catch (error) {
  ...
  _buildHierarchy(files) {
    ...
    for (const file of files) {
      const pathParts = file.path ? file.path.split('/') : file.name.split('/');
...
module.exports = FileUploadYjsService;
