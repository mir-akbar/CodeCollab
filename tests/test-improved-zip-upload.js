...
socket.on('connect', async () => {
  ...
  try {
    ...
    if (!fs.existsSync('./test-upload/test-with-system-files.zip')) {
      console.log('❌ Test ZIP file not found. Please ensure test-upload/test-with-system-files.zip exists');
      process.exit(1);
    }
    ...
    formData.append('file', fs.createReadStream('./test-upload/test-with-system-files.zip'));
