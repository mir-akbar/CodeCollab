/**
 * Updated Session Hooks with Token Authentication
 * Using the centralized apiClient with proper token management
 */
...
// Session API functions using the centralized apiClient
const sessionAPI = {
  ...
  createSession: async ({ sessionData, userEmail }) => {
    const response = await apiClient.post('/api/sessions', {
      ...sessionData,
      creator: userEmail,
      email: userEmail  // Add email to body as expected by API
    });
  ...
  inviteUser: async ({ sessionId, inviteeEmail, role, inviterEmail }) => {
    const response = await apiClient.post(`/api/sessions/${sessionId}/invite`, {
  ...
  removeParticipant: async ({ sessionId, participantEmail, userEmail }) => {
    const response = await apiClient.post(`/api/sessions/${sessionId}/remove-participant`, {
  ...
  promoteToOwner: async ({ sessionId, participantEmail, userEmail }) => {
    const response = await apiClient.post(`/api/sessions/${sessionId}/promote-owner`, {
      participantEmail: participantEmail,
      userEmail: userEmail
    });
  ...
  updateRole: async ({ sessionId, participantEmail, newRole, userEmail }) => {
    const response = await apiClient.post(`/api/sessions/${sessionId}/update-role`, {
      participantEmail: participantEmail,
      newRole: newRole,
